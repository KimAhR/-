...
// user라는 변수에 age 30, height 175 gender "male"라는 속성을 가진 객체.
...
// if문의 중첩
if (user.age >= 30) {
  if (user.gender === "female") console.log(user.name);
}
...
/*
  ...
  truthy : true처럼 평가되는 값.
    => 0이 아닌 수자, 문자열, 값이면 웬만하면 true처럼 동작.
  falsy : false처럼 평가되는 값. 0
    => 0 , undifined, null
 */
...
// user02 weight 프로퍼티가 없으므로 undefined가 출력되고,
// false 동작해서 없는 프로퍼티에 분기처리.
if (!user02.weight) {
