...
/*
    1. input 태그값 alert창 가져오기 => form 태그의 submit 이벤트가 발생했을 때.
...
movieForm.addEventListener("submit", (e) => {
  ...
  renderMovei02(inputTitle.value);
...
function renderMovei(movie) {
  document.querySelector(".movieZIP").innerHTML += `<li>
  <h2>${movie.title}</h2>
...
async function renderMovei02(title) {
  // Promise가 resolve 될 때가지 다음 코드를 실행하지 않는다. => 동기 코드처럼 동작한다.
  try {
