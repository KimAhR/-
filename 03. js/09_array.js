/*
  배열(array)
      ...
      - 배열의 총 크기를 배열의 길이라고 하며, 배열.length 프로퍼티 참조 할 수 있다.  
...
let spliceNum = arr.splice(0, 3, 1, 4); // 0번째 3개 삭제. 그 값을 배열로 변환. 원본 배열이 변경된다. 세번째 인자부터는 배열에 추가한다.
console.log(arr, spliceNum);

// slice : 배열의 중간요소를 복사.
let fruits = ["orange", "apple", "banana"];
let sliceNum = fruits.slice(0, 2); //0부터 2전까지 복사, 원본배열이 변경되지 않는다.
...
/*
  ...
  filter()
    - 각 요소를 순화하며 콜백함수를 실행하며, 특정 조건을 만족하는 요소만을 새로운 배열로 모아서 반환한다.
    - 콜백함수의 return 값이 boolean(true, false)값이어야 한다.

  map()
    - 각 요소를 순화하며 콜백함수를 실행하며, 요소의 값을 통해서 새로운 값을 만들어 내어 새로운 배열로 모아서 반환한다.
    - 해당 반복자의 콜백함수의 return값이 곧 새로운 배열의 요소가 된다.
    - 기본 배열은 변경되지 않는다.
*/
