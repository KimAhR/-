//JS는 정의한 함수에 정의된 매개변수보다 전달된 인자가 많아도 오류가 발생x
...
function sumTotal() {
  // arguments : 전달 받은 모든 인자를 유사배열 형태로 담는다.
...
console.log(...numArr); // console.lgo(1,2,3,4,5). 값 5개를 펼친 효과.
console.log(Math.max(...numArr)); // 전달된 이자 중에 가장 큰 수를 변환.
console.log(getTotal(...numArr));

let newArr = [0, ...numArr, 10]; // 기존 배열을 중간에 복사하는 효과
console.log(newArr);

// 객체 스프레드 : 객체의 속성들을 펼치는 효과. => 객체 안에 펼처서 사용.
//           => 주로 객체를 복사할 때 사용. 얕은 복사만 된다.
//              즉, 객체의 속성값이 또 객체면 그건 참조복사가 일어난다.
...
// 비구조화 할당(detructuring) : 배열이나 객체의 값을 변수에 할당할 때 간단하게 표현할 수 있다.
...
let fruits = ["orange", "apple", "banana"]; // 배열 구조화 할당. let [] 안에 변수이름은 자유롭게 지으면 배열의 순서대로 할당한다.
